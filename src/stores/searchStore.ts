...
export const useSearchStore = defineStore('counter', () => {
  ...
  const searchFlights = (origin: string, destiny: string, departureDate: string, returnDate: string) => {
    ...
    searchResults.value = flightsList.filter((flight => {
      ...
      if (flight.departure.toLowerCase().replace(RegExp('/', 'g'), '-') !== departureDate.toLowerCase().replace(RegExp('/', 'g'), '-')) {
        return false
      }
      if (flight.return.toLowerCase().replace(RegExp('/', 'g'), '-') !== returnDate.toLowerCase().replace(RegExp('/', 'g'), '-')) {
        return false
      }
